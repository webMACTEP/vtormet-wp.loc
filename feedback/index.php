	...
	'fields' => array(
		...
		'tell' => array(
			...
			'messages' => array(
				...
				'minlength' => 'Минимальная длинна поля <br>[ %1$s ] меньше допустимой - %2$s',
			)
		),
		'checkbox' => array(
			'title' => 'Согласие',
			...
			'messages' => array(
				'minlength' => '[ %1$s ] необходимо принять',
			)
		),
	),
	'cfg' => array(
		'charset' => 'utf-8',
		'subject' => 'Vtormet',
		'title' => 'Хочу получать цены',
		'ajax' => true,
		'validate' => true,
		'from_email' => 'admin@vtormet.ru',
		'from_name' => 'Администратор',
		'to_email' => '7286443@gmail.com, wmail@bk.ru',
		'to_name' => 'Roman, Admin',
		'geoip' => true,
		'referer' => true,
		'type' => 'html',
		...
		'okay' => 'Ваша заявка успешно отправлена.',
...
$form['form-order'] = array(
	'fields' => array(
		...
		'dem' => array(
			'title' => 'Нужен ли демонтаж?',
		),
		'vivoz' => array(
			'title' => 'Нужен ли вывоз?',
		),
		'gorod' => array(
			'title' => 'Город:',
		),
...
if (isset($form[$act])) {
	...
	foreach ($form['fields'] as $name => $field) {
		...
		if (isset($field['validate'])) {
			...
			// -1-
			if (
				isset($field['validate']['minlength']) &&
			...
			// -2-
			if (
				isset($field['validate']['maxlength']) &&
			...
			// -3-
			if (
				isset($field['validate']['preg']) && mb_strlen($rawdata) > 0 &&
	...
	if (count($error) == 0) {
		...
		$get_fromEmail = (isset($form['fields'][$form['cfg']['from_email']]) && isset($getdata[$form['cfg']['from_email']]['value']) && mb_strpos('@', $getdata[$form['cfg']['from_email']]['value']) === false) ? $getdata[$form['cfg']['from_email']]['value'] : ((mb_strpos('@', $form['cfg']['from_email']) !== false) ? $form['cfg']['from_email'] : 'admin@vtormet.ru');
